...
local droneCache = {}
local lastUpdate = 0
local UPDATE_INTERVAL = 0.033 -- 30 FPS для максимальной плавности
...
-- Цветовая палитра (современный черно-белый дизайн)
local COLORS = {
...
-- Оптимизированная функция для создания интерфейса
...
-- Оптимизированная функция для получения команды игрока
...
-- Функция для определения противоположной команды
...
-- Улучшенный поиск FPV дронов которые летают
local function findFPVDrones()
    local drones = {}
    
    -- Ищем основную модель дрона по GUID
    ...
    if droneModel and droneModel:IsA("Model") then
        ...
        for _, child in pairs(droneModel:GetChildren()) do
            if child:IsA("Model") then
                ...
                if rootPart then
                    -- Проверяем что дрон активен (высота над землей или имеет скорость)
    ...
    -- Дополнительный поиск дронов по всему workspace
    for _, obj in pairs(workspace:GetChildren()) do
        if obj:IsA("Model") then
            local nameLower = obj.Name:lower()
            -- Ищем по характерным названиям дронов
            ...
            -- Ищем по характерным частям дронов
            for _, part in pairs(obj:GetDescendants()) do
                if part:IsA("BasePart") then
...
-- Оптимизированный поиск врагов и дронов
...
-- Оптимизированная функция для применения X-Ray
local function applyXRay()
    if not Settings.Visuals.XRay then
        -- Восстанавливаем прозрачность только для измененных частей
    ...
    -- Применяем X-Ray только к новым частям
...
-- Оптимизированная функция для применения Chams
...
-- Оптимизированное создание обводки модели
...
-- Ультра-плавное обновление позиции ESP с максимальной стабильностью
local function smoothUpdatePosition(espFrame, targetPosition, alpha)
    ...
    -- Используем экспоненциальное сглаживание для максимальной плавности
    ...
    -- Ограничиваем максимальную скорость движения для стабильности
    local maxSpeed = 50 -- пикселей за кадр
...
-- Плавное изменение размера линии здоровья
...
-- Оптимизированное создание ESP элемента
local function createESP(target)
    ...
    -- Улучшенная панель информации с линией здоровья
...
-- Оптимизированное обновление ESP с ультра-плавностью и стабильностью
local function updateESP(targetData)
    ...
    if Settings.ESP.Enabled and onScreen and distance <= maxDistance then
        ...
        if infoPanel then
            -- Плавное обновление позиции с максимальной стабильностью
            local targetPosition = UDim2.new(0, screenPos.X - 60, 0, screenPos.Y - 50)
            
            -- Инициализация позиции если это первый кадр
            if targetData.SmoothPosition == UDim2.new(0, 0, 0, 0) then
                ...
                -- Ультра-плавное следование с разными коэффициентами для разных ситуаций
                local alpha = 0.3 -- Медленное сглаживание для максимальной плавности
                
                -- Если позиция сильно изменилась (резкий поворот камеры), используем более быстрое следование
                ...
                if posDelta > 100 then -- Большое изменение позиции
                    alpha = 0.6 -- Быстрее реагируем на резкие движения
                elseif posDelta > 50 then -- Среднее изменение
                    alpha = 0.4 -- Умеренная скорость
                end
            ...
            if healthBar then
                if targetData.Target.Type == "Enemy" then
                    ...
                    if humanoid and humanoid.MaxHealth > 0 then
                        ...
                        -- Плавное изменение здоровья
                        local healthAlpha = 0.5 -- Медленное изменение здоровья для плавности
                        ...
                        if healthFill then
                            -- Плавное изменение ширины линии здоровья
                            smoothHealthUpdate(healthFill, UDim2.new(targetData.HealthPercent, 0, 1, 0), 0.8)
                            
                            -- Градиент цвета от красного к зеленому
...
-- Оптимизированный основной цикл ESP
local function updateESPSystem()
    local currentTime = tick()
    
    -- Обновляем с фиксированным интервалом для максимальной плавности
    ...
    -- Оптимизация: используем пары только один раз
    ...
    -- Обновляем видимые цели
    for target, data in pairs(espCache) do
        updateESP(data)
    end
end

-- Функция для изменения FOV
...
local function createToggle(name, settingTable, settingKey, yPos, parentFrame)
    ...
    -- Функция анимации переключателя
...
-- Упрощенный редактор цветов без ручного ввода RGB
local function createColorEditor()
    ...
    -- Основные цвета (черный и белый внизу)
    local basicColors = {
        {Color = Color3.new(1, 0, 0), Name = "Красный"},    -- Красный
        {Color = Color3.new(0, 1, 0), Name = "Зеленый"},    -- Зеленый
        {Color = Color3.new(0, 0, 1), Name = "Синий"},      -- Синий
        {Color = Color3.new(1, 1, 0), Name = "Желтый"},     -- Желтый
        {Color = Color3.new(1, 0, 1), Name = "Пурпурный"},  -- Пурпурный
        {Color = Color3.new(0, 1, 1), Name = "Голубой"},    -- Голубой
        {Color = Color3.new(1, 1, 1), Name = "Белый"},      -- Белый
        {Color = Color3.new(0, 0, 0), Name = "Черный"}      -- Черный
    }
    
    local basicColorsFrame = Instance.new("Frame")
    basicColorsFrame.Size = UDim2.new(1, -20, 0, 40)
    basicColorsFrame.Position = UDim2.new(0, 10, 1, -120) -- Переместил вниз
    basicColorsFrame.BackgroundTransparency = 1
    basicColorsFrame.ZIndex = 101
    basicColorsFrame.Parent = colorEditor
    
    -- Создаем кнопки основных цветов
    ...
    -- Палитра цветов (основная сетка)
    local colorGrid = Instance.new("Frame")
    colorGrid.Size = UDim2.new(1, -20, 0, 200) -- Увеличил высоту
    colorGrid.Position = UDim2.new(0, 10, 0, 60)
    colorGrid.BackgroundTransparency = 1
    colorGrid.ZIndex = 101
    colorGrid.Parent = colorEditor
    
    -- Создаем цветовую сетку 8x8
    local cellSize = 25
    local spacing = 3
...
-- Функция для создания кнопки с шестерёнкой
...
-- Функция для создания интерфейса меню
local function createCheatMenu()
    ...
    -- Кнопка с шестерёнкой для открытия редактора цветов
    ...
    -- Оптимизированное обновление FPS
...
-- Оптимизированная функция для перезапуска скрипта при смерти
...
local function main()
    ...
    print("🟢 Winter ESP Enhanced загружена!")
